...
addSubmitEventListener=()=>{document.querySelector('#monster-form').addEventListener('submit',
a=>{a.preventDefault(),
    ...
    postNewMonster=a=>{
        ...
        fetch(b,c).then(d=>d.json())
        .then(d=>console.log('new monster',d))},
        clearForm=()=>{document.querySelector('#monster-form')
        .reset()},addNavListeners=()=>{
            let a=document.querySelector('#back'),
            b=document.querySelector('#forward');
            a.addEventListener('click',()=>{pageDown()}),
            b.addEventListener('click',()=>{pageUp()})},pageUp=()=>{
                page++,getMonsters(page)},pageDown=()=>{1<page?(page--,getMonsters(page)):alert('Aint no monsters here')},init=()=>{getMonsters(),createMonsterForm(),addNavListeners()};document.addEventListener('DOMContentLoaded',init);
